...
/**
 * @author YanMing
 * @title: IteratorToolFindId
 * @description:  迭代工具findId
 * @date 2022/4/9
 */

public class IteratorToolFindId {
    private static final Logger LOGGER = LoggerFactory.getLogger(IteratorToolFindId.class);
            ...
            if ("0".equals(parentId)) // 不存在父级，放到结果中
            {
                ...
                if (resultMap.containsKey(id)) // 存在对应id的数据
                {
                    ...
                    tempDataList.sort(Comparator
                            .comparing(IteratorToolFindId::comparingBySort));
                    // 替换合并结果
        ...
        for (Object data : allList)
        {
            ...
            for (Map.Entry<String, Object> entry : parentMap.entrySet())
            {
                ...
                childrenDatasList.sort(
                        Comparator.comparing(IteratorToolFindId::comparingBySort));
                // 父元素id在所有数据中能找到
        ...
        for (Map<String, Object> mu : allList)
        {
            ...
            if (Integer.valueOf(mu.get("parent").toString()) == pid)
            {
                ...
                getChildrenListForParent(allList,
                        Integer.valueOf(mu.get("id").toString()), children);
                children.add(mu);
            }
        }
        return children;
            ...
            throws Exception
    {
        ...
        if (!childNodeList.isEmpty())
        {
            ...
            // 排序子结点
            childNodeList
                    .sort(Comparator.comparing(IteratorToolFindId::comparing));
            if (parentNode instanceof Map<?, ?>)
            {
                @SuppressWarnings("unchecked")
