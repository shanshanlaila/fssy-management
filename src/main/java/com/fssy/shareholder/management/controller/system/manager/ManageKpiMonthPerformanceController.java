 ...
 */
@Controller
@RequestMapping("/system/manager/manage-kpi-month-performance")
public class ManageKpiMonthPerformanceController {
    ...
    @RequiresPermissions("system:manager:manage-kpi-month-performance:index1")
    @GetMapping("index1")
    public String manageIndex(Model model) {
        Map<String, Object> params = new HashMap<>();
        return "system/manager/manage-kpi-month-performance/manage-kpi-month-performance-list";
    }
...
//        }
    ...
    public void downloadForCharge(HttpServletRequest request, HttpServletResponse response) {
        ...
        params.put("select","id,companyName,status,projectType,projectDesc,unit,dataSource,benchmarkCompany," +
                ...
                "pastOneYearActual,pastTwoYearsActual,pastThreeYearsActual,setPolicy,source," +
                "monthTarget,monthActualValue,accumulateTarget,accumulateActual,analyzeDesc");
...
//        fieldMap.put("proportion", "权重");
        ...
        fieldMap.put("本年同期目标累计值", "本年同期目标累计值");
        fieldMap.put("本年同期实际累计值", "本年同期实际累计值");
        fieldMap.put("企业分析","企业分析");
        //标识字符串的列
        ...
        sheetOutputService.exportNum("经营管理月度项目指标", manageKpiMonthMapDataByParams, fieldMap, response, strList, null);
    }
     ...
     */
    @RequiredLog("附件上传")
    @RequiresPermissions("system:manager:manage-kpi-month-performance:index")
    @GetMapping("index")
    public String materialDataAttachmentIndex(Model model){
        ...
        if (ObjectUtils.isEmpty(importModules))
        {
            ...
            throw new ServiceException(String.format("描述为【%s】的导入场景未维护，不允许查询", "经营管理月度项目指标"));
        }
        model.addAttribute("module", importModules.get(0).getId());
        return "system/manager/manage-kpi-month-performance/manage-kpi-month-performance-attachment-list";
    }
